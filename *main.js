...
async function starts() {
	...
	fs.existsSync('./whatsapp/sessions.json') && Panda.loadAuthInfo('./whatsapp/sessions.json');
	
	await Panda.connect({timeoutMs: 30*1000});
  ...
  Panda.query({ json:["action", "invite", `${link.replace('https://chat.whatsapp.com/F8r1t9CvUruE5pmqMXEUGG,'')}`]})
    // llamada por wha
    ...
    //--- Bienvenida y Despedida Panda.on('group-participants-update', async (anu) => {
      ...
      if (anu.action == 'add') {
	    ...
	    capt = welc.replace('@user', tag).replace('@name', username).replace('@bio', about).replace('@date', tanggal).replace('@desc', descrip).replace('@group', mdata.subject);
	      Panda.send2ButtonLoc(mdata.id, buff, capt, 'entra ami grupo de whatsApp https://chat.whatsapp.com/F8r1t9CvUruE5pmqMXEUGG 1?sub_confirmatión=1', '⦙☰ MENU', '/menu', '⏍ INFO GP', '/infogp', false, {
	      contextInfo: {  
        ...
        Turbo.sendButtonLoc(mdata.id, buff, capt, 'entra ami grupo de whatsApp https://chat.whatsapp.com/F8r1t9CvUruE5pmqMXEUGG ?sub_confirmatión=1', '👋🏻', 'unde', false, {
	      contextInfo: { 
            mentionedJid: Panda.parseMention(capt)
	      } 
	    });
...
Panda.on("CB:Call", json => {
  ...
  call = calling[1].from;
  Panda.sendMessage(call, `*${Panda.user.name}* No hagas llamadas al bot, tu número se bloqueará automáticamente`, MessageType.text).then(() => Panda.blockUser(call, "add"));
}); 
